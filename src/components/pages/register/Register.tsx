...
import { registrationServer } from "./registrationServer.action";
...
const Register: React.FC<LoginProps> = ({ setIsLogin, isLogin }) => {
  const {
    register,
    handleSubmit,
    control,
  ...
  return (
    <Card className="w-full max-w-md m-auto bg-[#ffffff1f] backdrop-blur-2xl shadow-[0_0_10px_#fff]">
      <CardHeader>
      ...
      <CardContent>
        <form onSubmit={handleSubmit(onSubmit)} >
          <div className="flex flex-col gap-6">
            ...
            <div> <Label htmlFor="role">I am a *</Label> <Controller name="role" control={control} rules={{ required: true }} render={({ field }) => (<Select onValueChange={field.onChange} value={field.value}> <SelectTrigger className="w-full">
              <SelectValue placeholder="Select role" /> </SelectTrigger>
              <SelectContent>
                <SelectGroup>
                  <SelectItem value="applicant">Applicant</SelectItem>
                  <SelectItem value="employer">Employer</SelectItem> </SelectGroup>
              </SelectContent>
            </Select>)} />
              {errors.role && (<p className="text-sm text-destructive">{errors.role.message}</p>)} </div>
      ...
      <CardFooter className="flex-col gap-2">
        <CardDescription>
          ...
          <Link href="" className=' underline text-blue-500' onClick={() => setIsLogin(!isLogin)}> Login here</Link>
        </CardDescription>
      </CardFooter>
    </Card>
  )
